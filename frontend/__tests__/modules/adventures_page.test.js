...
const html = fs.readFileSync(path.resolve(__dirname, "../../pages/adventures/index.html"), "utf8");
...
describe("Adventure Page Tests", function () {
  ...
  it("fetchAdventures() - Makes a fetch call for /adventures API endpoint and returns an array with the adventures data", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(expect.stringContaining("?city=bengaluru"));
  ...
  it("addAdventureToDOM() - <a> tag links the adventure card correctly to the corresponding Adventure details page", function () {
    ...
    expect(document.getElementById("123456").href).toEqual(expect.stringContaining("/detail"));
    expect(document.getElementById("123456").href).toEqual(expect.stringContaining("?adventure=123456"));
  });
  ...
  it("filterFunction() - Returns an array of adventures, filtered by both duration and categories", function () {
    ...
    expect(output).toBeInstanceOf(Array);
    expect(output.map((a) => a.id).sort()).toEqual(expected.map((a) => a.id).sort());
  });
  ...
  it("saveFiltersToLocalStorage() - Adds the updated filter as a string to localstorage", function () {
    ...
    expect(window.localStorage.setItem).toHaveBeenCalledWith("filters", JSON.stringify(filters));
  });
