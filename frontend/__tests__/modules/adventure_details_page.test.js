...
const html = fs.readFileSync(path.resolve(__dirname, "../../pages/adventures/detail/index.html"), "utf8");
jest.dontMock("fs");
...
describe("Adventure Detail Page Tests", function () {
  ...
  it("Check if fetch call for the adventure details was made and data was received", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(expect.stringContaining("?adventure=123"));
  });
  ...
  it("Tries adding a Adventure Details - Park", function () {
    ...
    expect(document.getElementById("adventure-name").innerHTML).toBe(adventure.name);
    expect(document.getElementById("adventure-subtitle").innerHTML).toBe(adventure.subtitle);
    expect(document.getElementsByClassName("activity-card-image").length).toBe(adventure.images.length);
  });
  ...
  it("Check if bootstrap gallery is working", function () {
    ...
    expect(document.getElementsByClassName("carousel-item").length).toBe(images.length);
    expect(document.getElementsByClassName("carousel-item active").length).toBe(1);
  ...
  it("Check if reservation banner is displayed", function () {
    ...
    expect(document.getElementById("reserved-banner").style.display).toBe("block");
